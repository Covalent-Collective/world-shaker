...
- [x] **OpenRouter image generation model choice** — RESOLVED 2026-04-26: `bytedance-seed/seedream-4.5` selected as default avatar model. Rationale: $0.04/image fixed cost (predictable), multi-image composition fits stylized avatars built from `extracted_features`, B-grade quality bar acceptable per R8 (avatar is not S-grade surface). Set in Vercel Production/Preview/Development envs and local `.env`. Swap path documented in `lib/avatar/generate.ts` — env-only flip to `google/gemini-2.5-flash-image` (Nano Banana) or `black-forest-labs/flux.2-klein-4b` if alpha shows quality/cost regression. Validation gate: first 100 generations checked for safety-filter rejection rate ≤5% and p95 latency ≤60s.
